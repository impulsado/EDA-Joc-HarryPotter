...
struct PLAYER_NAME : public Player {
  ...
  enum Priority {
    ...
    WIZARD_ALLIED_CLOSE = 0,
    WIZARD_ALLIED_MID = 9,
    WIZARD_ALLIED_FAR = 17,
    // -- MAG ENEMIG --
    WIZARD_ENEMY_WIZARD_CLOSE_HIGH = 1,
  ...
  unordered_map<int, Pos> last_wizard_pos;
  
  bool initialized = false;
  bool solved = false;  // Controlar si hem trobar solucio spell
  Pos centre_de_massa;
  Pos last_ghost_pos = {-1, -1};
  ...
  void initPTWizards() {
    ...
    // -- MAG ALIAT PER SALVAR --
    probability_table_wizards[ALLIED] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- MAG ENEMIG --
    probability_table_wizards[ENEMY_WIZARD] = vector<vector<Priority>>(dist_ranges, vector<Priority>(prob_ranges, NOT_DEFINED));
    ...
    // -- FANTASMA ENEMIG --
    probability_table_wizards[ENEMY_GHOST] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- LLIBRE --
    probability_table_wizards[BOOK] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- VOLDEMORT --
    probability_table_wizards[VOLDEMORT] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- EMPTY --
    probability_table_wizards[EMPTY] = vector<vector<Priority>>(dist_ranges, vector<Priority>(prob_ranges, NOT_DEFINED));
  ...
  void initPTGhost() {
    ...
    // -- MAG ENEMIG --
    probability_table_ghost[ENEMY_WIZARD] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- LLIBRE --
    probability_table_ghost[BOOK] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- VOLDEMORT --
    probability_table_ghost[VOLDEMORT] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
    ...
    // -- EMPTY --
    probability_table_ghost[EMPTY] = vector<vector<Priority>>(dist_ranges, vector<Priority>(1, NOT_DEFINED));
  ...
  // Determinar si aproximar-se o allunyar-se quan fem el BFS
  Dir bestDir(const UnitType& t, const Pos& danger_pos, const Dir& initial_dir, const Priority& prio, const Pos& initial_pos) {
    // === BASE CASE
    // Si no hem d'allunyar-se, retornar direccio inicial
    if (smove_forwards.find(prio) == smove_forwards.end()) return initial_dir;

    // Calcular direcció oposada
    int dx = danger_pos.j - initial_pos.j;
    int dy = danger_pos.i - initial_pos.i;

    // Seleccionar possibles direccions
    vector<Dir> possible_dirs;

    if (t == Ghost) {
      if (dy > 0 && dx > 0) possible_dirs.push_back(UL); // Amunt Esquerra
      if (dy > 0 && dx < 0) possible_dirs.push_back(RU); // Amunt Dreta
      if (dy < 0 && dx > 0) possible_dirs.push_back(LD); // Avall Esquerra
      if (dy < 0 && dx < 0) possible_dirs.push_back(DR); // Avall Dreta
    }
    ...
    // Si no trobem alguna valida, la inicial i au
    return initial_dir;
  }
  ...
  void wizardBFS(const Unit& u, PQM& pqmovements, const int& max_bfs_depth) {
    ...
    while (!pqnodes.empty()) {
      ...
      // -- MAG ALIAT PER SALVAR --
      if (element_board[act.i][act.j].elem == ALLIED) {
        /*
        si no m'estic converint i l'altre si i la distancia fins a ell es menor a les rondes que li queden --> anar
        si m'estic convertint i l'altre no i la distancia fins a ell es menor a les rondes que li queden --> anar
        */
        Cell temp_cell = cell(act.i, act.j);
        Unit temp_allied_unit = unit(temp_cell.id);

        if (!is_transforming && temp_allied_unit.is_in_conversion_process()) {
          if (temp_allied_unit.rounds_pending < dist) continue;
        ...
        else if (is_transforming && !temp_allied_unit.is_in_conversion_process()) {
          if (u.rounds_pending < dist) continue;
      ...
      // -- LLIBRE --
        ...
        if (is_transforming && dist > u.rounds_pending) continue;
  ...
  Dir calcularDireccio(const Pos& desde, const Pos& hacia) {
    if (hacia.i < desde.i) return Up;
    if (hacia.i > desde.i) return Down;
    if (hacia.j < desde.j) return Left;
    return Right;
  }
  ...
  Movement findBestMove(int id) {
    ...
    else if (smy_units.size()>=50) max_bfs_depth = 35; 
    else if (smy_units.size()>=40) max_bfs_depth = 40;   
    ...
    while (!possible_movements.empty()) {
      ...
      if (movement_board[best_move.p.i][best_move.p.j].id == -1) {
        ...
        if (unit_act.type == Ghost) last_ghost_pos = unit_act.pos;
        else last_wizard_pos[id] = unit_act.pos;
        return best_move;
      }
      // Determinar qui està més aprop (El més lluny torna a ficar-se en el set)
        ...
        if (element_board[best_move.p.i][best_move.p.j].elem == ENEMY_WIZARD && movement_board[best_move.p.i][best_move.p.j].id != ghost(me())) {
          if (unit_act.type == Ghost) last_ghost_pos = unit_act.pos;
          else last_wizard_pos[id] = unit_act.pos;
          return best_move;
        }
    ...
    // Si no he trobat res interessant, anar cap on estiguin totes
    Dir dir_al_centre = calcularDireccio(unit_act.pos, centre_de_massa);
    Pos nueva_pos = unit_act.pos + dir_al_centre;
    if (pos_ok(nueva_pos) && element_board[nueva_pos.i][nueva_pos.j].elem != WALL && movement_board[nueva_pos.i][nueva_pos.j].id == -1) {
      if (unit_act.type == Wizard && nueva_pos != last_wizard_pos[unit_act.id]) {
        Movement temp;
        temp.p = nueva_pos;
        temp.id = id;
        temp.d = dir_al_centre;
        movement_board[temp.p.i][temp.p.j] = temp;
        if (unit_act.type == Ghost) last_ghost_pos = unit_act.pos;
        else last_wizard_pos[unit_act.id] = unit_act.pos;
        return temp;
      }
    }
    ...
    if (unit_act.type == Wizard) {
      for (auto d : dirs_wizard)  {
        ...
        else if (new_pos == last_wizard_pos[unit_act.id]) continue;
  ...
  Pos calcularCentreMassa() {
    int sum_i = 0, sum_j = 0, count = 0;
    for (const auto& id : smy_units) {
      Unit u = unit(id);
      sum_i += u.pos.i;
      sum_j += u.pos.j;
      count++;
