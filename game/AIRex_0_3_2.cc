...
/*
IMPROVEMENTS:
- En low, millor aliat que llibre. Altrament no.
  ...
  void wizardBFS(const Unit& u, PQM& pqmovements, const int& max_bfs_depth) {
    ...
    for (const auto& dir : dirs_wizard) {
      ...
      if (pos_ok(new_pos) && element_board[new_pos.i][new_pos.j].elem != WALL) {
        dist_pes[new_pos] = element_board[new_pos.i][new_pos.j].pes;
    ...
    while (!pqnodes.empty()) {
      ...
      // === BASE CASE
      if (node.pes > dist_pes[node.pos]) continue;  // Ja tenim una ruta més curta
      ...
      for (const auto& dir : dirs_wizard) {
        Pos new_pos = act + dir;
        if (pos_ok(new_pos) && element_board[new_pos.i][new_pos.j].elem != WALL) {
          if (dist_pes.find(new_pos) == dist_pes.end()) dist_pes[new_pos] = INT32_MAX;
          int new_cost = dist_pes[act] + element_board[new_pos.i][new_pos.j].pes;
          if (new_cost >= dist_pes[new_pos]) continue;
          dist_pes[new_pos] = new_cost;
