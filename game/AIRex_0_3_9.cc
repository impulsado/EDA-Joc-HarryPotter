...
struct PLAYER_NAME : public Player {
  ...
  Movement findBestMove(int id) {
    ...
    if (pos_ok(nueva_pos) && element_board[nueva_pos.i][nueva_pos.j].elem != WALL && movement_board[nueva_pos.i][nueva_pos.j].id == -1 && nueva_pos != last_ghost_pos) {
      Movement temp;
      temp.p = nueva_pos;
      temp.id = id;
